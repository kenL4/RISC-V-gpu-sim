...
InstructionFetch::InstructionFetch(InstructionMemory *im, LLVMDisassembler *disasm): im(im), disasm(disasm) {
...
void InstructionFetch::execute() {
    ...
    log("Instruction Fetch", "Warp " + std::to_string(warp->warp_id) + 
        " will execute instruction " + disasm->getOpcodeName(inst.getOpcode()));
};
